...
- **批次查询**：输入打印批次号 → 按**打印顺序**列出该批次每单（编码 / 件数 / 货位 / 在架），再给一份「按货位汇总」；也可直接读 ERP 导出的 xlsx / csv
- **手机拣货页**（`/pick`）：单条模式一次只显示一个待拣商品，**语音播报**（分区 + 编码逐位念 + 件数）、拣货完成 / 无货、进度持久化（退出页面、重启程序都能续上）、按分区 A→B→C→D 排序
...
2. 默认「单条」模式：一次只显示一条（分区徽标 + 编码 + **需拣件数** + 货位/在架），进页即语音播报
3. 点「拣货完成」/「无货」→ 自动切到下一条并播报；顶栏显示进度（已拣 / 无货 / 待拣 / 总件数）
4. 进度存在程序端 SQLite 的 `pick_batch` 表：**退出页面、关浏览器、重启程序都能续上**，点「结束批次」才结束
5. 排序默认按**分区 A→B→C→D**（可在页面上切回按打印序号）；「按货位汇总」看按货位的归类
...
GET /api/pick/mark?batch=<>&g=<组序>&state=done|short|pending   标记/撤销
